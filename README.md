...
The full dataset is quite large, and the entire process (from the first to the last step) may take hours. Therefore, we directly provide the input files for each step.  **Risk Classification (deform_risk_classi(whole).py)** can be run directly for quick review after setting up the environment.yml.
